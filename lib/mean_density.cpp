...
static inline size_t idx(const int ix, const int iy, const int iz, const size_t nc)
{
  size_t iix= (ix + nc) % nc;
  size_t iiy= (iy + nc) % nc;
  size_t iiz= (iz + nc) % nc;

  return (iix*nc + iiy)*nc + iiz;
}

void mean_density_set_from_grid(Grid const * const grid,
				const Float fac,
				const size_t np,
